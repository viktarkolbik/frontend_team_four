...
export class InterviewDialogComponent {
  ...
  constructor(
    ...
    public data: { users: UserParseDate[]; role: string }
  ) {
    ...
    users.forEach(user =>
      user.userTimeSlots.forEach(timeSlot => {
        if (this.datePicker.value) {
          if (this.equalDate(timeSlot.startDate, this.datePicker.value)) {
  ...
  filterUsers(): void {
    ...
    if (this.selectedTime) {
      this.users = this.users.filter(user =>
        user.userTimeSlots.some(
          timeSlot =>
            (this.datePicker.value
              ? this.equalDate(this.datePicker.value, timeSlot.startDate)
              : true) &&
