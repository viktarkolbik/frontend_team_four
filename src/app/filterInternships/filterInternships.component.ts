...
export class FilterInternshipsComponent implements OnChanges {
  ...
  @Output() filterInternships: EventEmitter<Internship[]> = new EventEmitter<
    Internship[]
  >();
  ...
  constructor() {}
  ngOnChanges() {
    const setCities = this.internships
      .map(internship => internship.locations)
      .flat();
    const setCitiesArray = new Set(
      setCities.map(location => location.city.name)
    );
    this.cities = Array.from(setCitiesArray);
    const setTechnologiesArray = this.internships.map(
    ...
    this.countries = Array.from(
      new Set(setCities.map(location => location.country.name))
    );
    // @ts-ignore
  ...
  updateTrainings() {
    this.filterInternships.emit(
      this.internships.filter(internship => {
        let condition = false;
        const filters = Object.values(this.filters);
        for (const filter of filters) {
          const filterCriteria: Criterion[] = Object.values(filter.criteria);
          filter.isChecked = filterCriteria.some(
            criterion => criterion.isChecked
          );
  ...
  removeFilter(data: any, condition: string) {
    this.filters[data].criteria[condition].isChecked =
      !this.filters[data].criteria[condition].isChecked;
